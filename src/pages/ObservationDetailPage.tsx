...
export function ObservationDetailPage({ observationId, startEditing = false }: { observationId: string; startEditing?: boolean }) {
  ...
  return (
    <main className="content-page observation-detail">
      ...
      <section className="panel detail-section" id="verification"><div className="section-heading"><p className="eyebrow">現實驗證</p><h2>{verificationStatusLabels[status]}</h2></div><dl className="summary-list"><div><dt>驗證需求</dt><dd>{observation.verification.requirement === "required" ? "需要驗證" : observation.verification.requirement === "not_required" ? "不需要驗證" : "目前不確定"}</dd></div><div><dt>可驗證命題</dt><dd>{observation.verification.prediction || "未設定"}</dd></div><div><dt>期限</dt><dd>{observation.verification.dueDate || "未設定"}</dd></div><div><dt>判定標準</dt><dd>{observation.verification.criteria || "未設定"}</dd></div><div><dt>證據來源</dt><dd>{observation.verification.evidenceSources?.join("、") || "未選擇"}</dd></div></dl>{status === "pending" || status === "overdue" ? <a className="secondary-button button-link inline-button" href="#/pending">進入現實驗證</a> : null}</section>
    </main>
  );
}
